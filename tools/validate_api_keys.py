...
def validate_nvd_api_key():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"❌ NVD API request failed with status code: {response.status_code}"
            )
            return False
...
def validate_otx_api_key():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"✅ AlienVault OTX API key is valid (user: {user_data.get('username', 'unknown')})"
            )
            return True
            ...
            logger.error(
                f"❌ AlienVault OTX API request failed with status code: {response.status_code}"
            )
            return False
    except Exception as e:
...
def validate_openai_api_key():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"❌ OpenAI API request failed with status code: {response.status_code}"
            )
            return False
    except Exception as e:
...
def validate_google_api_key():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"❌ Google Gemini API request failed with status code: {response.status_code}"
            )
            return False
    except Exception as e:
...
def main():
    ...
    print("=" * 50)
    print(
        f"✅ Valid Keys:    {', '.join(results['valid']) if results['valid'] else 'None'}"
    )
    print(
        f"❌ Invalid Keys:  {', '.join(results['invalid']) if results['invalid'] else 'None'}"
    )
    print(
        f"⚠️ Missing Keys:  {', '.join(results['missing']) if results['missing'] else 'None'}"
    )
    print("=" * 50)
